...
`evidence/artifact-hashes.txt` authenticates the committed publication files.
Native executable hashes inside `evidence/upstream-execution.json` identify one
specific run and are not expected to match across architectures, compilers,
linkers, source paths, or OpenSSL builds. The same evidence file records the local
toolchain and platform used for that run.

GitHub Actions performs a clean-checkout reproduction without uploading fetched
upstream source or generated binaries. `scripts/verify_publication.py` fails closed
if forbidden raw artifacts, large files, or recognizable private-key/token formats
...
- `THIRD_PARTY_NOTICES.md` — upstream provenance and licensing boundaries.
- `scripts/verify_publication.py` — tracked-tree publication safety gate.
...
The root MIT license applies only to original coldhax files. Fetched Coldcard,
libngu, and MicroPython source remains under its upstream terms; in particular,
Coldcard carries Commons Clause terms and libngu carries a Bitcoin-only
restriction. See `THIRD_PARTY_NOTICES.md` before reuse or redistribution.
